    ...
    try {
        ...
        if (mousePos.x >= 0 && lastPlayedType === 'processed') {
             ...
             if (hoverGR < -0.01) {
                 ...
                 ctx.beginPath();
                 const crossWidth = 10;
                 ctx.moveTo(mousePos.x - crossWidth, grY); ctx.lineTo(mousePos.x + crossWidth, grY);
                 ctx.stroke();
                 ctx.strokeStyle = 'rgba(236, 72, 153, 0.4)'; 
                 ctx.beginPath(); ctx.moveTo(0, grY); ctx.lineTo(width, grY); ctx.stroke();
             } else {
                 if (hoverGrRef) hoverGrRef.current = 0;
             }
        }
    } catch (e) {
...
const Waveform = ({ 
    ...
    return (
        <div 
            ref={containerRef} 
            className="flex-1 relative bg-slate-900 border-2 border-slate-800 rounded-xl shadow-inner overflow-hidden flex cursor-crosshair select-none" 
