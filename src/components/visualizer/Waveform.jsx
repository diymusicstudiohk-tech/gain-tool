...
export const drawMainWaveform = ({
    ...
    try {
        ...
        // Grid — horizontal dB lines with gradient fade (ref: claude-eq-tool)
        const FADE_DISTANCE = 160;
        const fadeL = Math.min(FADE_DISTANCE / width, 0.4);
        const fadeR = 1 - fadeL;

        // 0dB centre line (very subtle)
        ...
        for (let db = -5; db >= -60; db -= 5) {
            ...
            // Vertical edge fade: lines near top/bottom of canvas get dimmer
            ...
            // Top line
            if (topFade > 0) {
                ctx.globalAlpha = Math.max(0, topFade);
                ctx.strokeStyle = lineGrad;
                ctx.beginPath(); ctx.moveTo(0, yTop); ctx.lineTo(width, yTop); ctx.stroke();
            }
            // Bottom line (mirror)
            if (botFade > 0) {
                ctx.globalAlpha = Math.max(0, botFade);
                ctx.strokeStyle = lineGrad;
                ctx.beginPath(); ctx.moveTo(0, yBot); ctx.lineTo(width, yBot); ctx.stroke();
            }
        }
        ctx.globalAlpha = 1;
